    ...
    <div className="p-4 h-full flex flex-col relative">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto pb-20 no-scrollbar">
        ...
        {!loading && customers.map(c => {
          ...
          return { ...c, cardTime, cardTimeStatus, isKeyDeal, age };
        }).sort((a, b) => {
          const aTimeout = a.cardTimeStatus === 'urgent' || a.cardTime === '已超时';
          const bTimeout = b.cardTimeStatus === 'urgent' || b.cardTime === '已超时';
...
function CustomerItem({ id, name, task, sourceChannel, isKeyDeal, age, stage, time, urgent, status, color = "red", timeStatus = "urgent" }: any) {
  ...
  return (
    <Link to={`/customers/${id}`} className={`block bg-white rounded-3xl p-5 shadow-sm relative ${borderClass}`}>
